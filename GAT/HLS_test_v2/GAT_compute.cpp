...
//#pragma HLS array_partition variable=all_attention_coefficients_buffer dim=1 complete
    ...
    for (int nh = 0; nh < num_of_heads; nh++) {
        for (int n1 = 0; n1 < num_of_nodes; n1++) {  
            for (int fout = 0; fout < FEATURE_OUT; fout++) {
                FM_TYPE sum = 0;
                int it = 0;
                while (it < num_of_nodes / FEATURE_OUT) {
                    for (int i = 0; i < FEATURE_OUT; i++) {
#pragma HLS PIPELINE
                        all_attention_coefficients_buffer[i] = all_attention_coefficients[nh][n1][it * FEATURE_OUT + i];
                    }
                    
                    for (int n2 = 0; n2 < FEATURE_OUT; n2++) {
                        sum += all_attention_coefficients_buffer[n2] * nodes_features_proj[it * FEATURE_OUT + n2][nh][fout];
                    }
                ...
                for (int i = 0; i < FEATURE_OUT; i++) {
#pragma HLS PIPELINE
                    if (i < num_of_nodes % FEATURE_OUT)
                        all_attention_coefficients_buffer[i] = all_attention_coefficients[nh][n1][it * FEATURE_OUT + i];
                    else
                        all_attention_coefficients_buffer[i] = 0;
                }
                ...
                out_nodes_features_prep[n1][nh * FEATURE_OUT + fout] = sum;
            }
        }
    }
    */
