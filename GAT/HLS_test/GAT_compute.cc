...
FM_TYPE out_nodes_features_concat[MAX_NODE][HEAD_NUM * FEATURE_OUT];
    ...
    for (int nh = 0; nh < num_of_heads; nh++) {
        for (int n1 = 0; n1 < num_of_nodes; n1++) {  
            for (int fout = 0; fout < num_out_features; fout++) {
                FM_TYPE sum = 0;
                for (int n2 = 0; n2 < num_of_nodes; n2++) {
                    sum += all_attention_coefficients[nh][n1][n2] * nodes_features_proj[n2][nh][fout];
                }
                out_nodes_features[n1][nh][fout] = sum;
            }
        }
    }
}
...
#pragma HLS array_partition variable=skip_proj_weight dim=3 complete
    ...
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int dim_out = 0; dim_out < num_of_heads * num_out_features; dim_out++) {
            ...
            out_nodes_features[nd][dim_out / num_out_features][dim_out % num_out_features] += sum;
        }
    }
}
...
#pragma HLS array_partition variable=out_nodes_features dim=3 complete
    // not concat -> out_nodes_features = out_nodes_features.mean(dim=self.head_dim)
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int fout = 0; fout < num_out_features; fout++) {
            FM_TYPE sum = 0;
            for (int nh = 0; nh < num_of_heads; nh++) {
                sum += out_nodes_features[nd][nh][fout];
            }
...
void compute_concat(int num_of_nodes, int num_of_edges, int num_in_features, int num_out_features, int num_of_heads, int layer) {

#pragma HLS inline off
#pragma HLS array_partition variable=out_nodes_features_concat dim=2 complete
    // concat -> out_nodes_features = out_nodes_features.view(-1, self.num_of_heads * self.num_out_features)
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int dim_out = 0; dim_out < num_of_heads * num_out_features; dim_out++) {
        	out_nodes_features_concat[nd][dim_out] = out_nodes_features[nd][dim_out / num_out_features][dim_out % num_out_features];
        }
    }
}
...
#pragma HLS array_partition variable=out_nodes_features_skip_concat_bias dim=2 complete
    ...
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int dim_out = 0; dim_out < num_of_heads * num_out_features; dim_out++) {
            if (out_nodes_features_concat[nd][dim_out] <= 0) {
            	FM_TYPE sum = hls::exp(out_nodes_features_concat[nd][dim_out]) - (FM_TYPE)1;
                out_nodes_features_skip_concat_bias[nd][dim_out] = sum;
            } else {
                out_nodes_features_skip_concat_bias[nd][dim_out] = out_nodes_features_concat[nd][dim_out];
            }
        }
    }
}
...
#pragma HLS inline off
    ...
    if (layer == 4) {
        ...
        compute_concat(num_of_nodes, num_of_edges, num_in_features, num_out_features, num_of_heads, layer);
...
void load_misc_weights(
    ...
    int* num_heads_per_layer_in, int* num_features_per_layer_in
    )
{
#pragma HLS inline off
    ...
    for (int i = 0; i < LAYER_NUM + 1; i++) {
        num_heads_per_layer[i] = num_heads_per_layer_in[i];
    }

    for (int i = 0; i < LAYER_NUM + 1; i++) {
        num_features_per_layer[i] = num_features_per_layer_in[i];
    }
    */
}
...
{
#pragma HLS inline off
#pragma HLS array_partition variable=connectivity_mask dim=1 complete
...
void GAT_compute_one_graph(
    int* node_feature_in, int* edge_list_in, int* graph_attr, int* num_heads_per_layer_in, int* num_features_per_layer_in, FM_TYPE* task_tb,
    WT_TYPE graph_pred_linear_weight_fixed[NUM_TASK * FEATURE_OUT],
    WT_TYPE graph_pred_linear_bias_fixed[NUM_TASK],
...
#pragma HLS INTERFACE m_axi depth=100000 port=num_heads_per_layer_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=num_features_per_layer_in offset=slave bundle=mem
...
#pragma HLS bind_storage variable=out_nodes_features_concat type=RAM_2P impl=bram
    ...
    */
    if (is_first) {
		...
		load_misc_weights(graph_pred_linear_weight_fixed, graph_pred_linear_bias_fixed, num_heads_per_layer_in, num_features_per_layer_in);
    }
