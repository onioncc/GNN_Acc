...
void CONV_0(int* in_nodes_feature, int num_of_nodes, int num_of_edges, int num_in_features, int num_out_features, int num_of_heads)
{
    //printf("\n---- Computing CONV 0 ----\n");
    ...
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int dim_out = 0; dim_out < num_of_heads * num_out_features; dim_out++) {
            for(int dim_in = 0; dim_in < num_in_features; dim_in++) {
                nodes_features_proj[nd][dim_out / num_out_features][dim_out % num_out_features] += in_nodes_feature[nd * num_in_features + dim_in] * gat_net_0_linear_proj_weight[dim_out][dim_in];
            }
        }
    }
    ...
    memset(scores_source, 0, MAX_NODE * MAX_NODE * sizeof(float));
    ...
    memset(scores_target, 0, MAX_NODE * MAX_NODE * sizeof(float));
    for(int nh = 0; nh < num_of_heads; nh++) {
        for(int n = 0; n < num_of_nodes; n++) {
            for(int fout = 0; fout < num_out_features; fout++) {
    ...
    // out_nodes_features = out_nodes_features7.permute(1, 0, 2)
    ...
    if (num_in_features == num_out_features) {
        ...
        for (int n1 = 0; n1 < num_of_nodes; n1++) { 
            for (int nh = 0; nh < num_of_heads; nh++) {
                for (int fout = 0; fout < num_out_features; fout++) {
                    out_nodes_features[n1][nh][fout] += in_nodes_feature[n1 * num_out_features + fout];
                }
            }
        }
    } else {
        ...
        for(int nd = 0; nd < num_of_nodes; nd++) {
            for(int dim_out = 0; dim_out < num_of_heads * num_out_features; dim_out++) {
                for(int dim_in = 0; dim_in < num_in_features; dim_in++) {
                    out_nodes_features[nd][dim_out / num_out_features][dim_out % num_out_features] += in_nodes_feature[nd * num_in_features + dim_in] * gat_net_0_skip_proj_weight[dim_out][dim_in];
                }
            }
        }
    }
    ...
    // if self.bias is not None: out_nodes_features += self.bias
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int dim_out = 0; dim_out < num_of_heads * num_out_features; dim_out++) {
            out_nodes_features_skip_concat_bias[nd][dim_out] += gat_net_0_bias[dim_out];
        }
    }
...
void CONV_1(int* in_nodes_feature, int num_of_nodes, int num_of_edges, int num_in_features, int num_out_features, int num_of_heads)
{
    //printf("\n---- Computing CONV 0 ----\n");
    ...
    // if self.bias is not None: out_nodes_features += self.bias
    for(int nd = 0; nd < num_of_nodes; nd++) {
        for(int dim_out = 0; dim_out < num_out_features; dim_out++) {
            out_nodes_features_skip_concat_bias[nd][dim_out] += gat_net_1_bias[dim_out];
        }
    }
...
void GAT_compute_one_graph(int* node_feature, int* edge_list, int* graph_attr, int* num_heads_per_layer, int* num_features_per_layer)
{
    ...
    ////////////// CONV 0 //////////////////////////////////
    CONV_0(node_feature, num_of_nodes, num_of_edges, num_features_per_layer[0] * num_heads_per_layer[0], num_features_per_layer[1], num_heads_per_layer[1]);
    ////////////// CONV 1 //////////////////////////////////
    CONV_1(node_feature, num_of_nodes, num_of_edges, num_features_per_layer[1] * num_heads_per_layer[1], num_features_per_layer[2], num_heads_per_layer[2]);
