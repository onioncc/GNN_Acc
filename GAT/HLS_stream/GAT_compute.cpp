...
FM_TYPE out_nodes_features_sum[FEATURE_OUT];
...
void compute_all_attention_coefficients(int num_of_nodes, int num_of_edges, int num_in_features, int num_out_features, int num_of_heads, int layer, hls::stream<FM_TYPE> &emb_vec) {
...
#pragma HLS PIPELINE
                if (all_scores[nh][n1][n2] != (FM_TYPE)0) {
                    ...
                    emb_vec.write(hls::exp(all_scores[nh][n1][n2]) / attention_coefficients_sum[nh][n1]);
                } else {
...
void compute_out_nodes_features(int num_of_nodes, int num_of_edges, int num_in_features, int num_out_features, int num_of_heads, int layer, hls::stream<FM_TYPE> &emb_vec) {
...
#pragma HLS inline off
    ...
    // NOTE: PREVIOUS PARALLISM TRICK IS NOT APPLIED HERE!!!
    for (int nh = 0; nh < num_of_heads; nh++) {
        for (int n1 = 0; n1 < num_of_nodes; n1++) {  

            for (int n2 = 0; n2 < num_of_nodes; n2++) {
                
                FM_TYPE a = emb_vec.read();
...
void dataflow_region(int num_of_nodes, int num_of_edges, int num_in_features, int num_out_features, int num_of_heads, int layer) {
#pragma HLS dataflow

    hls::stream<FM_TYPE> emb_vec;
#pragma HLS STREAM variable=emb_vec depth=160000
    
    compute_all_attention_coefficients(num_of_nodes, num_of_edges, num_in_features, num_out_features, num_of_heads, layer, emb_vec);
	compute_out_nodes_features(num_of_nodes, num_of_edges, num_in_features, num_out_features, num_of_heads, layer, emb_vec);
}
...
#pragma HLS inline off
    ...
    dataflow_region(num_of_nodes, num_of_edges, num_in_features, num_out_features, num_of_heads, layer);
...
#pragma HLS inline off
    ...
    // reset all_scores value
    memset(all_scores, 0, HEAD_NUM * num_of_nodes * num_of_nodes * sizeof(FM_TYPE));
}
