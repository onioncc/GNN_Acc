...
void compute_edge_embedding_and_message_passing(int num_of_nodes, int num_of_edges, WT_TYPE h[MAX_NODE][EMB_DIM], int edge_attr[MAX_EDGE][EDGE_ATTR], int layer)
{
#pragma HLS inline off

//#pragma HLS array_partition variable=message dim=2 complete
#pragma HLS array_partition variable=edge_embedding_weight complete
#pragma HLS array_partition variable=edge_attr dim=2 complete
#pragma HLS array_partition variable=edge_list factor=2 cyclic
...
#pragma HLS pipeline
            ...
            // accumulate the embedding vector for edge [u -> v]
            WT_TYPE msg = edge_embed + h[u][dim];
            if(msg < 0) msg = 0.0;
            message[v][dim] += norm[e] * msg;
        }
    }
}
...
{
#pragma HLS array_partition variable=convs_weight dim=2 complete
...
#pragma HLS PIPELINE
            ...
            linear_layer_dim_in: for(int dim_in = 0; dim_in < MLP_0_IN; dim_in++) {
                ...
                tmp += node_embedding[nd][dim_in] * convs_weight[layer][dim_out][dim_in];
            }
            linear_out[nd][dim_out]  += tmp;
        }
    }

    compute_edge_embedding_and_message_passing(num_of_nodes, num_of_edges, linear_out, edge_attr, layer);
...
#pragma HLS INTERFACE m_axi depth=100000 port=convs_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=convs_bias_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=convs_root_emb_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=bn_weigh_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=bn_bias_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=bn_mean_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=bn_var_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=node_embedding_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=edge_embedding_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=graph_pred_weights_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=100000 port=graph_pred_bias_in offset=slave bundle=mem
...
//#ifdef _PRINT_
    printf("Final graph prediction in HLS:\n");
    for(int tsk = 0; tsk < NUM_TASK; tsk++) {
        printf("%.7f\n", task[tsk].to_float());
    }
    printf("\nGCN computation done in HLS.\n");
//#endif
