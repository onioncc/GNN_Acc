...
from .pyg_to_dgl import convert
...
class PlanetoidDGL(torch.utils.data.Dataset):
    def __init__(self, data, norm='norm', pos_enc_dim=0):
        self.data = [convert(g) for g in data]
        self.graph_lists = []
        for g in self.data:
    ...
    def __getitem__(self, idx):
        """
            ...
            (dgl.DGLGraph, int)
                ...
                And its label.
        """
        return self.graph_lists[idx], torch.Tensor([0])
...
class PlanetoidDataset(Dataset):
    def __init__(self, name, pos_enc_dim=0, norm='none', verbose=True):
        ...
        self.name = name
        self.dataset = Planetoid(f'../../graphs/large_graphs', name)

        self.train = PlanetoidDGL(self.dataset, norm=norm, pos_enc_dim=pos_enc_dim)
        self.val = self.train
