...
void MLP(int num_of_nodes, int layer)
{
...
#pragma HLS pipeline
        for(int dim = 0; dim < EMB_DIM; dim++) {
            message[nd][dim] = message[nd][dim] + (1 + _eps) * node_embedding[nd][dim];
        }
    }
...
#pragma HLS pipeline
    ...
    for(int dim = 0; dim < MLP_1_OUT; dim++) {
        for(int nd = 0; nd < num_of_nodes; nd++) {
#pragma HLS pipeline
            MLP_one_node_one_dim(dim, nd, message, node_embedding, layer);
        }
    }
