...
void compute_edge_embedding_and_message_passing(FM_TYPE h_node[MAX_NODE][EMB_DIM], int number_of_nodes, int num_of_edges, int layer)
{
#pragma HLS inline off

#pragma HLS array_partition variable=edge_attr dim=2 complete
#pragma HLS array_partition variable=edge_embedding_table dim=1 complete

#pragma HLS array_partition variable=edge_embedding_table dim=2 cyclic factor=16
#pragma HLS array_partition variable=h_node dim=2 cyclic factor=16
#pragma HLS array_partition variable=message dim=2 cyclic factor=16

    
    //memset(edge_embedding, 0, MAX_EDGE * EMB_DIM * sizeof(FM_TYPE));
    memset(message, 0, MAX_NODE * EMB_DIM * sizeof(FM_TYPE));
    loop_edge: for(int e = 0; e < num_of_edges; e++) {
        
        int u = edge_list[e*2];     // source node id
        int v = edge_list[e*2+1];   // target node id

        loop_dim: for(int dim = 0; dim < EMB_DIM; dim += EMB_BATCH) {
#pragma HLS pipeline II=2

        	FM_TYPE edge_emb[EMB_BATCH];
            memset(edge_emb, 0, EMB_BATCH * sizeof(FM_TYPE));
            loop_dd: for(int dd = 0; dd < EMB_BATCH; dd++) {
                ...
                for(int ef = 0; ef < EDGE_ATTR; ef++) {
                    ...
                    emb_value = edge_embedding_table[addr + e_f][dim + dd];
                    edge_emb[dd] += emb_value;
                }   

                //// Message passing
                // accumulate the embedding vector for edge [u -> v]
                FM_TYPE msg = edge_emb[dd] + h_node[u][dim + dd];
                if(msg < 0) msg = 0.0;
                message[v][dim + dd] += msg;
            }
        }
    }
...
void load_mlp_weights_one_layer(int layer, WT_TYPE* gnn_node_mlp_1_weights_fixed, WT_TYPE* gnn_node_mlp_1_bias_fixed,
                                           WT_TYPE* gnn_node_mlp_2_weights_fixed, WT_TYPE* gnn_node_mlp_2_bias_fixed)
{
#pragma HLS inline off
...
#pragma HLS inline off
    ...
    compute_edge_embedding_and_message_passing(h_node, num_of_nodes, num_of_edges, layer);
...
#pragma HLS inline off
	...
	for(int nd = 0; nd < num_of_nodes; nd++) {
		for(int dim = 0; dim < EMB_DIM; dim++) {
			h_node_sum[dim] += h_node[nd][dim];
        }
    }

	for(int dim = 0; dim < EMB_DIM; dim++) {
		h_graph[dim] = h_node_sum[dim] / num_of_nodes;
	}
...
#pragma HLS inline off
    ...
    for(int e = 0; e < num_of_edges; e++) {
        for(int e_t = 0; e_t < EDGE_ATTR; e_t++) {
            edge_attr[e][e_t] = edge_attr_in[e * EDGE_ATTR + e_t];
        }
    }
