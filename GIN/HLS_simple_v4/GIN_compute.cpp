...
void MLP(FM_TYPE mlp_in[MAX_NODE][EMB_DIM], FM_TYPE mlp_out[MAX_NODE][EMB_DIM], FM_TYPE h[MAX_NODE][EMB_DIM], int num_of_nodes, int layer)
{
#pragma HLS inline off
    ...
    memset(mlp_out, 0, num_of_nodes * MLP_OUT_MAX * sizeof(FM_TYPE));
    for(int dim = 0; dim < MLP_1_OUT; dim++) {
        for(int nd = 0; nd < num_of_nodes; nd++) {
#pragma HLS pipeline
            MLP_one_node_one_dim(dim, nd, mlp_in, mlp_out, layer);
        }
    }
...
#pragma HLS inline off
    ...
    memset(edge_embedding, 0, num_of_edges * EMB_DIM * sizeof(FM_TYPE));
    for(int e = 0; e < num_of_edges; e++) {
