...
def gen_graph_bin(gname, G):
    ...
    f = open('./random_graphs/%s_node_feature.bin' % gname, 'wb')
    packed = struct.pack('i'*len(data), *data)
    f.write(packed)
    f.close()

    data = list(edge_list)
    f = open('./random_graphs/%s_edge_list.bin' % gname, 'wb')
    packed = struct.pack('i'*len(data), *data)
    f.write(packed)
    f.close()
    ...
    f = open('./random_graphs/%s_edge_attr.bin' % gname, 'wb')
    packed = struct.pack('i'*len(data), *data)
    f.write(packed)
    f.close()

    f = open('./random_graphs/%s_info.txt' % gname, 'w')
...
for i in range(0, 9):
    ...
    # each list contains another list, storing the percentage of nodes with degree larger than 4
...
for itr in range(0, 1000):
    ...
    G = nx.gnm_random_graph(n, e, seed=None, directed=True)
    ...
    # print("percentage of node with degree larger than 4: %.2f" % perct)
    #print(G.edges)
    #print(G.nodes)
...
for i in range(2, 9):
    ...
    for j in range(0, 10):
        ...
        f = open('./random_graphs/g_list_degree_%d_percent_%d.txt' % (i, j*10), 'w')
